...
shinyUI(fluidPage(theme = shinytheme('flatly'),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      ,br(),
      h4("This code simulates one-sample t-tests where data generated from a sample with a specified mean and sd is tested against the population mean IQ score of 100, and plots the p-value distribution."),
    ...
    mainPanel(
      ...
      h4("Which p-values you can expect is completely determined by the statistical power of the study, or the probability that you will observe a significant effect, if there is a true effect. The statistical power ranges from 0 to 1. Let's get started by performing some simulations. You can set the number of simulations (the higher the number the more accurate the results, but the longer it takes – leaving them at 10000 for this assignment should be ok). This shiny app simulates one-sample t-tests. The idea is that we simulate IQ scores for a group of people (you can change the sample size, which is set to 26 by default). We know the standard deviation of IQ scores is 15 (specified as well by moving the slider). For now, we will set the mean IQ score in the simulated group to 106, which we will compare to the average IQ score of all people (which is known to be 100 – that's how IQ tests are normalized). We are testing if the people in our simulated sample have an IQ that differs from the average (and we know the correct answer is 'yes', because we made it so in the simulation)."),
      h4("In the simulation, we generate n (by default 26) normally distributed IQ scores with a mean of M (106 by default) and a standard deviation of 15. We then perform a one-sample t-test against the population mean, which in an IQ test is known to be an IQ score of 100 (so the null-hypothesis is that the data from our simulation sample does not differ from 100), and store the p-value. Obviously, when we set the mean in the samples we simulate to a value that is not 100 (such as the default of 106) we know that there is a true difference. The nice thing about a simulation is that you know the truth in the world you are simulating."),
      h4("The simulation can take a while to run on the server (especially when simulating more than 10000 studies), and will display a plot of all observed p-values."),
      ...
      h4("Lets take a look at just the p-values below 0.05. The goal of this part of this assignment is to cure you from a bi-polar p-value disorder, where people incorrectly think all p-values > 0.05 are support for the null-hypothesis, and all p-values below 0.05 are support for the alternative hypothesis. Bear with me for the next few steps – it will be worth it. Change the number of bars to 100. We will now get 1 bar for p-values between 0 and 0.01, one bar for p-values between 0.01 and 0.02, and 100 bars in total. The grey line will now indicate the frequency of p-values when the null hypothesis is true, where every bar contains 1% of the total number of p-values. We only want to look at p-values below 0.05, and we will cut off the plot at 0.05. Click the button that will zoom the graph to the p < 0.05 area. Instead of seeing all p-values between 0 and 1, we will only see p-values between 0 and 0.05. The simulation will update (still with a mean of 100). We see the same uniform distribution, but now every bar contains 1% of the p-values, so the p-value distribution is very flat and almost impossible to see. The grey line now clearly gives the frequency for each bar, assuming the null hypothesis is true."),
      ...
      h4("You can check the answers you gave on the Multiple Choice questions in my free Coursera MOOC ", a("Improving Your Statistical Inferences", href="https://www.coursera.org/learn/statistical-inferences"))
    )
    )
  )
))
